...
- オフライン中や電波の弱い場所での撮影
  ...
  - 共有に失敗した撮影は捨てる (自分の写真と採点を進捗にも履歴にも残さない)。誰もクリアしていない扱いに戻るので、同じスポットをもう一度撮影できる
    - Rules に拒否された場合 (ルームが終わったあとなど) も同じく捨てる
    - 共有の途中でアプリが終了した撮影 (写真はあるが発見者がいない) も、次にルームに戻ったときに捨てる。ただし、そのスポットのクリアがサーバにあれば (共有は届いていた。先着に負けた場合を含む) 捨てない。`clears` を読めなければ、判断できないので捨てない
  - 共有中は、そのスポットを撮影できない (同じ発見を並行して送らないため)
...
3. サムネのアップロードとクリアの作成が **30 秒**以内に終わらなければ、クリアは失敗にする (6.1 のとおり撮り直してもらう)
   ...
   - 時間切れのあとにサムネのアップロードが終わっても、クリアは作成しない (失敗と伝えた撮影を発見にしない)
   - 既知の制限: クリアの作成中に時間切れになった場合は取り消せないため、時間切れのあとにサーバへ届いてクリアが作成されることがある。その場合は `clears` の通知で自分が発見者として反映される (手元の撮影は捨てているので、自分のサムネを表示する)。通知の前にもう一度撮影したときも自分のクリアとして扱い、サムネはその撮影のものにする (発見日時は最初に届いたクリアの時刻)
...
- ルームに戻る
  ...
  - ルームがもう終わっていれば (結果画面を閉じる前にアプリを終了した場合など)、「結果を見る」として結果画面を開く。結果画面の「ホームへ戻る」で消える
