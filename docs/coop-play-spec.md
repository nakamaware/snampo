...
| 抜けたまま戻らない (抜ける、キル、電池切れ) | アプリの起動時と履歴画面を開いたときに、保持期限 (7 日) 内で未確定の協力プレイ履歴について、`clears` と `rooms` を 1 回だけ取得する (監視はしない)。不足分を取得し、発見者とサムネを更新する。ルームが `finished` か、遊べる期限を過ぎていれば「確定」にして、以後は取りにいかない |
