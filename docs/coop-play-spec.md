...
rooms/{roomCode}/clears/{spotId}
  ...
  clearedAt: timestamp
  thumbPath: string?             // サムネの Storage パス。アップロード失敗時は null で、後から埋める
  deleteAt: timestamp
```
...
- オフライン中の撮影
  - クリアの書き込みは SDK が端末に溜めておき、復帰したら送信する
  - その間に他の人がクリアしていたら、Rules に拒否されるので、上記の「先に発見」の表示にする

### 6.2 サムネのアップロード

1. 撮影してクリアと判定されたら、まずサムネ (長辺 480px、JPEG) を `thumbs/{spotId}/{uid}.jpg` にアップロードする
2. アップロードが終わったら、`thumbPath` を入れて `clears/{spotId}` を作成する
3. **15 秒**以内に終わらなければ、`thumbPath` なしでクリアを先に作成する (発見の同期を優先する)
   - この間は、撮影した本人の画面に「発見を共有中…」を表示する
4. 発見は、クリアを作成する前に端末に保存したキューに積む。キルされても消えないようにする
   - クリアを作成する前にキルされた場合は、次の機会にクリアを作り直す
     - 作り直す時点で先に他の人が発見していた場合や、ルームが終わっていた場合は、「発見を共有できませんでした」とエラーを表示して破棄する
     - クリアの送信を待つのは 30 秒まで。終わらなければ (オフラインなど) キューに残し、次の機会に送り直す
   - クリアを作成済みでサムネだけが失敗した場合は、サムネを再送する
   - 次のネットワーク復帰、アプリの復帰、起動のタイミングで送り直す
   - 送り直すのは遊べる期限 (12 時間) まで
5. 再送が成功したら、`thumbPath` を後から埋める
   - 送信待ちだった自分のクリアが先に届いていて `thumbPath` が空の場合も、後から埋める
6. 他の端末は、`clears` の変更通知 (監視中) か、「8. 同期」の処理で後から取得する
...
- ルームを抜ける
  ...
  - 抜けた人が同じルームに入り直すと、`leftAt` を null に戻す (ニックネームも入り直した時点のものにする)。グレー表示と人数の計算は、入り直した時点で元に戻る
- 期限切れ
...
| パス | 読み取り | 作成 | 更新 | 削除 |
|---|---|---|---|---|
| `rooms/{code}` | メンバーまたはホスト。入室前の存在確認 (`get`) は認証済みなら可 | 認証済みで `hostId == auth.uid`。create-only で、期限の値は作成時刻から計算した値と一致すること | ホストのみ、遊べる期限内。変更できるフィールドは `status`、`settings`、`missionRef`、`spotIds`、`generationError`、`finishReason`、`startedAt`、`finishedAt`。例外として、全スポットがクリアされたときはメンバーも `playing` → `finished` (`allCleared`) に更新できる | 不可 |
| `members/{uid}` | メンバー | 本人 (`uid == auth.uid`) のみ。遊べる期限内 | 本人のみ。`nickname` と `leftAt` だけ変更できる | 不可 |
| `clears/{spotId}` | メンバー | メンバーで、`status == playing` かつ遊べる期限内。`clearedBy == auth.uid`。`spotId` が `spotIds` に含まれていること | `clearedBy` 本人だけが、`thumbPath` を null から値へ 1 回だけ変更できる。遊べる期限内 | 不可 |
...
| ケース | 取得のタイミング |
|---|---|
| ずっと参加している | `clears` の通知 (追加と `thumbPath` の更新) が届いた時点で、サムネを取得してキャッシュする |
| 一時的に抜けて、ルームに戻った | 戻ったときの `clears` の全件スナップショットとキャッシュを比べ、不足分をまとめて取得する |
| 抜けたまま戻らない (抜ける、キル、電池切れ) | アプリの起動時と履歴画面を開いたときに、保持期限 (7 日) 内で未確定の協力プレイ履歴について、`clears` と `rooms` を 1 回だけ取得する (監視はしない)。不足分を取得し、発見者とサムネを更新する。ルームが `finished` で発見者のサムネが全部そろったか、遊べる期限を過ぎていれば「確定」にして、以後は取りにいかない (`finished` のあとも、サムネは遊べる期限までは再送で届くため) |
| 7 日以上アプリを開かなかった | データが消えているので、取得できなかった分はプレースホルダを表示する |
| サムネのアップロードに失敗し、再送も期限内に成功しなかった | `thumbPath` がないので、プレースホルダを表示する |
...
| サムネ | 先にアップロードし、15 秒でタイムアウトし、再送する | 画像が見つからない状態を避けつつ、発見の同期を優先するため |
