...
- PR は 1 本
- 協力プレイで発見者の結果を見せるため、ソロとも共通のプレイと履歴の画面も作り直す
  - スポットの結果: 判定の色の帯と、距離・向きのずれの図で見せる
  - プレイ結果と履歴の詳細: 発見数・かかった時間・判定の内訳のまとめ (`MissionRecap`) と、スポットの写真のグリッド (履歴の詳細は見本と撮った写真の対)
  - 履歴一覧: スポットの写真の帯で見分けられるようにし、月ごとに見出しを付ける
  - 撮影画面で見本を見ながら撮れるようにし、見本と撮った写真を上下に並べて一緒に拡大できる全画面の表示を作る (拡大していなければ下へのスワイプで閉じる)
  - ライセンスを、ホームの右下から設定画面に移す
...
| ルームに戻る | アプリのキルや電池切れなどのあと、同じ `uid` で進行中のルームへ復帰すること (Home の「ルームに戻る」や途中参加。アプリを開いたまま電波が切れて戻った場合は含まない) |
| その場で届いた発見 | 電波がつながっている間に 1 件だけ届いた他の人の発見。電波が戻ったときや、ルームに戻ったときに、それまでの発見をまとめて受け取るのは「追いつく」 |
...
rooms/{roomCode}/clears/{spotId}
  ...
  judgement: map?                // 発見者の採点 (他の人も同じ結果を見るため)。古いクリアにはない
    ...
    zoomLevel: number?           // 撮影したときのズームの倍率 (0 より大きく 100 以下。古いアプリは書かない)
  deleteAt: timestamp
```
...
- `generating`
  ...
  - 失敗したら `waiting` に戻し、`generationError` を設定する
    - ホストには「ミッションの生成に失敗しました。設定を変えて再試行してください」と表示する。失敗の詳細 (`generationError`) は、開発用のビルド (dev) でだけ添える
...
- **1 人のクリアで全員のクリア**になる。クリア済みのスポットは誰も撮影できない (撮影ボタンを無効にする)
  - カメラ画面を開いている間に他の人が発見したら、撮影できなくし、「先に○○さんが発見しました」と表示してカメラ画面を閉じる (採点中なら、下の先を越された場合として扱う)
- 先着勝ち: `clears/{spotId}` は作成のみ
- 発見者の採点 (判定・距離・向き・撮影位置) もクリアに入れて共有する。他の人のスポットの結果画面、プレイ結果、履歴でも、発見者と同じ結果を見られる
  - 同時に撮影して作成に失敗した側 (先を越された側) は、自分の写真と採点を捨てる (進捗にも履歴にも残さない)。他の人が発見したスポットと同じ扱いにする
  - そのうえで「先に○○さんが発見しました」と表示し、撮影直後の結果画面には発見者の写真と採点を出す (発見者とそのサムネを、`clears` の通知を待たずに反映してから開く)
- 他の人がクリアしたとき
  - 「○○さんがスポット N を発見!」のバナーを表示する
  - スポットカードに発見者とサムネを表示する
  - その場で届いた発見なら、全員の画面で、そのスポットの結果画面 (発見者の写真と採点) を自動で開く
    - Mission 画面が前面にあるときだけ開く。撮影中やスポットの結果画面を見ている間などは割り込まず、バナーだけにする (結果画面は重ねて開かない)
    - そのスポットのサムネの取得を試し終えてから開く (取得できなければ、時間切れを含めてプレースホルダ)。ほかのスポットのサムネの取り直しは待たない
    - 最後のスポットは、ルームが `finished` になったときに開き、閉じると結果画面へ移る。自分で撮影したスポットや、その場で届いた発見でないスポット (ルームに戻ったときに追いついたなど) なら開かずに結果画面へ移る
      - 発見者と、そのスポットのサムネの取得を試し終えてから開く (ほかのスポットのサムネの取り直しは待たない)。待つのは **10 秒**までにし、終わらなければ (電波が弱くサムネの取得が終わらないなど) 反映できた分で開く (サムネが届いていなければプレースホルダ)。最後のスポットの発見まで反映できていなければ、開かずに結果画面へ移る
      - 新しい `clears` が届いたら、前の反映はほかのスポットのサムネの取り直しを打ち切り、新しい反映に任せる (最後のスポットの発見を出すのを遅らせないため)
  - 電波が切れて戻ったときに追いついた発見 (電波が切れていた間の発見) や、一度に複数届いた発見は、バナーだけにして結果画面を開かない
    - 複数なら「他のメンバーが N か所のスポットを発見!」の 1 つにまとめる
    - 電波が切れると `clears` は端末のキャッシュの値になり、戻るとサーバの値になる。キャッシュの値のあとに届いたサーバの値を、電波が戻ったときとして扱う
  - ルームに戻ったとき (途中参加や復帰) に追いついた、それまでの発見では、バナーも結果画面も出さない
    - `clears` は、最初に端末のキャッシュの値が届き、アプリを終了していた間の発見はそのあとのサーバの値で届く。サーバの最初の値までを追いつくまでとして扱う
- オフライン中や電波の弱い場所での撮影
  ...
  - 共有に失敗した撮影は捨てる (自分の写真と採点を進捗にも履歴にも残さない)。誰もクリアしていない扱いに戻るので、同じスポットをもう一度撮影できる
    ...
    - 共有の途中でアプリが終了した撮影 (写真はあるが発見者がいない) も、次にルームに戻ったときに捨てる。ただし、そのスポットの自分のクリアがサーバにあれば (共有は届いていた) 捨てず、自分の写真と採点を履歴に残す。他の人のクリアがあれば (先着に負けた) 捨てる。`clears` は端末のキャッシュではなくサーバから読み、読めなければ (時間内に返事が来ない場合を含む)、判断できないので捨てない。そのあと、サーバの最新の `clears` が届いたとき (電波が戻ったときなど) に決め直す
...
- 入室できるのは `waiting`、`generating`、`playing` のとき
  - `playing` からの途中参加も可。既存の `clears` を読み込んで追いつく (追いつくまでの発見は知らせない。6.1)
...
- ルームに戻る
  ...
  - ルームがもう終わっていれば (結果画面を閉じる前にアプリを終了した場合など)、「結果を見る」として結果画面を開く。結果画面の「ホームへ戻る」で消える
    - この結果画面でもルームを監視し、アプリを終了していた間の発見を反映して、共有の途中で終了した撮影の扱いを決める (6.1)
    - 「ホームへ戻る」を押すと、撮影の扱いを決め終えるまでボタンを押せなくし、確かめている間の表示 (インジケータ) を出す
      - 決める撮影がなければ待たない (サムネの取得など、ほかの反映は待たない)。決めるのを待つのは **20 秒**までにする
    - 撮影の扱いを決められなければ (電波がない、電波が弱くて時間内にサーバから返事が来ないなど)、「ホームへ戻る」でも進捗を片付けずに残してホームへ戻る (サーバに届いていた撮影の写真を消さないため)。「撮影を確かめられなかったため、結果を残しました」と表示する。「結果を見る」は残り、電波のある場所で開き直すと決め直す
  - 同じ `uid` で状態を保ったまま復帰できる。ホストの場合はホスト権限も戻る (`hostId == uid`)
  - ルームに戻ったときに追いついた発見は知らせない (6.1)。アプリを開いたまま電波が切れて戻ったときは、ルームに戻る扱いにせず、その間の発見をバナーで知らせる
...
- サムネの取得は 1 枚ごとに **10 秒**まで待つ。時間内に取得できなければ失敗と同じく扱い、次の同期で取り直す (電波が弱いと取得が終わらず、ほかの反映や、それを待つ画面の遷移を止め続けてしまうため)

### 8.1 履歴 (Drift)

- 協力プレイの履歴は、`playing` に遷移した時点で「進行中」として作成する
- その後は `roomCode` をキーに upsert する。ソロは既存どおり、完了時に 1 回だけ保存する
- スキーマの変更 (schemaVersion 3 → 4 へのマイグレーション。1 回で次の列をすべて足す)
  - `mode` に `coop` を追加する
  - 協力プレイの情報 (`mission_histories`)
    - `roomCode`
    - 同期の状態 (進行中、確定)
    - 自分がホストだったか
    - メンバー一覧 (`uid` とニックネーム)
    - 遊べる期限とデータの保持期限 (確定の判定と同期の対象を決めるため)
  - スポットごとの情報 (`history_spots`)
    - 撮影したときのズームの倍率 (ソロも含む)
    - `spotId`
    - 発見者の `uid` とニックネーム
    - サムネのローカルパス
    - 発見者の採点 (判定、位置と方角の誤差、撮影した位置と方角、ズームの倍率)
    - クリア済みかどうか (途中終了の場合、未クリアのスポットがある。既存の履歴はクリア済みにする)
