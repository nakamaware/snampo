...
- オフライン中や電波の弱い場所での撮影
  ...
  - 共有に失敗した撮影は捨てる (自分の写真と採点を進捗にも履歴にも残さない)。誰もクリアしていない扱いに戻るので、同じスポットをもう一度撮影できる
    ...
    - 共有の途中でアプリが終了した撮影 (写真はあるが発見者がいない) も、次にルームに戻ったときに捨てる。ただし、そのスポットのクリアがサーバにあれば (共有は届いていた。先着に負けた場合を含む) 捨てず、自分の写真と採点を履歴に残す。`clears` は端末のキャッシュではなくサーバから読み、読めなければ、判断できないので捨てない。そのあと、サーバの最新の `clears` が届いたとき (電波が戻ったときなど) に決め直す
...
- ルームに戻る
  ...
  - ルームがもう終わっていれば (結果画面を閉じる前にアプリを終了した場合など)、「結果を見る」として結果画面を開く。結果画面の「ホームへ戻る」で消える
    ...
    - 撮影の扱いを決められなければ (電波がないなど)、「ホームへ戻る」でも片付けずにホームへ戻る (サーバに届いていた撮影の写真を消さないため)。「結果を見る」は残り、開き直すと決め直す
