...
- オフライン中や電波の弱い場所での撮影
  ...
  - 共有に失敗した撮影は捨てる (自分の写真と採点を進捗にも履歴にも残さない)。誰もクリアしていない扱いに戻るので、同じスポットをもう一度撮影できる
    ...
    - 共有の途中でアプリが終了した撮影 (写真はあるが発見者がいない) も、次にルームに戻ったときに捨てる。ただし、そのスポットのクリアがサーバにあれば (共有は届いていた。先着に負けた場合を含む) 捨てず、自分の写真と採点を履歴に残す。`clears` は端末のキャッシュではなくサーバから読み、読めなければ、判断できないので捨てない
