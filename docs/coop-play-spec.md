...
3. サムネのアップロードとクリアの作成が **30 秒**以内に終わらなければ、クリアは失敗にする (6.1 のとおり撮り直してもらう)
   ...
   - 既知の制限: 時間切れのあとにサーバへ届いてクリアが作成されることがある。その場合は `clears` の通知で自分が発見者として反映される (手元の撮影は捨てているので、自分のサムネを表示する)。通知の前にもう一度撮影したときも自分のクリアとして扱い、サムネはその撮影のものにする (発見日時は最初に届いたクリアの時刻)
