...
4. 発見は、クリアを作成する前に端末に保存したキューに積む。キルされても消えないようにする
   ...
   - ルームを抜けたら、そのルームのクリアを作成していない送り直しは破棄する (抜けた人はクリアを作成できないため)。作成済みのクリアのサムネの再送は続ける
   - 既知の制限: 最初の撮影の送信中 (サムネの 15 秒、オフラインならその間ずっと) に送り直しが走ると、同じ発見を並行して送ることがある。先着勝ちと「自分のクリアなら作成済みとして扱う」処理で結果は揃うので許容する (サムネのアップロードなどが 2 回ずつ走るだけ)
...
- ルームを抜ける
  ...
  - 抜けた人は、クリアの作成と `finished` (`allCleared`) への更新ができない (Rules)。抜ける前に積んだクリアの送り直しも破棄する (サムネの再送は続ける)
...
| `clears/{spotId}` | メンバー | 抜けていない (`leftAt` のない) メンバーで、`status == playing` かつ遊べる期限内。`clearedBy == auth.uid`。`spotId` が `spotIds` に含まれていること | `clearedBy` 本人だけが、`thumbPath` を null から値へ 1 回だけ変更できる。遊べる期限内 | 不可 |
