...
def test_奇数個の座標リストから中間地点を計算できること() -> None:
...
def test_偶数個の座標リストから中間地点を計算できること() -> None:
...
def test_1つの座標のみの場合はその座標を返すこと() -> None:
...
def test_2つの座標の場合は2番目の座標を返すこと() -> None:
...
def test_長いルートでも正しく中間地点を計算できること() -> None:
