...
def test_get_directions_同じ引数で複数回呼び出したときにキャッシュが効くこと() -> None:
...
def test_get_directions_異なる引数で呼び出したときにそれぞれ別のリクエストが発生すること() -> None:
...
def test_get_street_view_metadata_同じ引数で複数回呼び出したときにキャッシュが効くこと() -> None:
    """同じ引数で複数回呼び出したときに、APIリクエストが1回だけになることを確認"""
...
def test_get_street_view_metadata_異なる引数で別リクエストが発生すること() -> None:
    """異なる引数で呼び出したときに、それぞれ別のリクエストが発生することを確認"""
...
def test_get_street_view_image_同じ引数で複数回呼び出したときにキャッシュが効くこと() -> None:
    """同じ引数で複数回呼び出したときに、APIリクエストが1回だけになることを確認"""
    coordinate = Coordinate(latitude=35.6812, longitude=139.7671)
...
def test_get_street_view_image_異なる引数で別リクエストが発生すること() -> None:
    """異なる引数で呼び出したときに、それぞれ別のリクエストが発生することを確認"""
    coordinate1 = Coordinate(latitude=35.6812, longitude=139.7671)
    coordinate2 = Coordinate(latitude=35.6895, longitude=139.6917)
    image_size = ImageSize(width=640, height=640)
...
def test_get_street_view_image_異なる画像サイズで別リクエストが発生すること() -> None:
